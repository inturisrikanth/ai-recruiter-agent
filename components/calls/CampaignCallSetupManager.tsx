...
export function CampaignCallSetupManager({
  campaignId,
  templates,
  initialConfig,
}: {
  ...
  const [formInitial, setFormInitial] = useState<CallConfigurationDraft | null>(initialConfig);
  ...
  async function onAttachSelected() {
    ...
    try {
      ...
      setFormInitial({
        ...
        callNotes: safeNotes.length ? safeNotes : null,
      });
      setFormKey((k) => k + 1);
      router.refresh();
    } catch (e) {
  ...
  return (
    <div className="grid gap-6">
      <div>
        <header className="rounded-3xl bg-white p-5 shadow-sm ring-1 ring-zinc-200/70 sm:p-6">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
            <div>
              <div className="text-sm font-semibold text-zinc-900">Reusable templates</div>
              <div className="mt-1 text-sm text-zinc-600">
                Pick a saved call setup template to attach to this campaign.
              </div>
            </div>

            <div className="flex flex-wrap gap-2">
              <Link href="/calls/templates/new" className={tableActionButtonClass()}>
                New call template
              </Link>
            </div>
          </div>
        ...
        {selectionError ? (
          <div className="mt-6 rounded-3xl bg-rose-50 p-5 text-sm text-rose-800 ring-1 ring-rose-200/70 sm:p-6">
            <div className="font-semibold text-rose-900">Couldn’t attach template</div>
        ...
        {attachedTemplateName ? (
          <div className="mt-6 rounded-3xl bg-emerald-50 p-5 text-sm text-emerald-900 ring-1 ring-emerald-200/70 sm:p-6">
            ...
            <div className="mt-1 text-emerald-800">
              Attached <span className="font-semibold">{attachedTemplateName}</span>. You can customize below and save.
            </div>
          </div>
        ) : null}
                    ...
                    <tr>
                      <td colSpan={6} className="px-4 py-10">
                        <div className="mx-auto max-w-xl text-center">
                          ...
                          <div className="mt-4">
                            <Link href="/calls/templates/new" className={campaignActionButtonClass(false)}>
                              New call template
                            </Link>
                          </div>
                        </div>
        ...
        <div className="mt-4 flex justify-end">
          <button
            ...
            className={campaignActionButtonClass(isAttaching || templates.length === 0)}
          >
      ...
      <CallConfigurationForm key={formKey} campaignId={campaignId} initial={formInitial} />
    </div>
  );
}
