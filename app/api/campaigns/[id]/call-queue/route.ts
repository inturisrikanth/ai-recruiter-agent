...
export async function POST(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (existingSession?.id) {
    ...
    return NextResponse.json({ sessionId: String(existingSession.id), created: false }, { status: 200 });
  }
  ...
  return NextResponse.json({ sessionId: callSessionId, created: true }, { status: 201 });
}
